...
def get_evidence_from_rationales(
    post_id: str, rationale_tensor: torch.Tensor, tokenizer: Any, k: int = 5
) -> List[Dict[str, Any]]:
    """
...
def get_actual_token_text(
    input_ids: torch.Tensor, tokenizer: Any, start_idx: int, end_idx: int
) -> str:
    """
...
def convert_entry_to_eraser_ground_truth(
    entry: Dict[str, Any], tokenizer: Any, k: int = 5
) -> Dict[str, Any]:
    """
    ...
    evidence_list = get_evidence_from_rationales(
        post_id, rationale_tensor, tokenizer, k=k
    )
    ...
    for evidence in evidence_list:
        ...
        actual_text = get_actual_token_text(
            input_ids, tokenizer, start_idx, end_idx
        )  # Get actual text for the token span, for example "nigg", "#er" (in tokeinzed number form) will be converted to "nigger"
        evidence["text"] = (
            actual_text if actual_text else f"tokens_{start_idx}_{end_idx-1}"
        )
...
def create_eraser_ground_truth_from_df(
    df,
    tokenizer: Any,
    output_dir: str,
    model_name: str,
    task_type: str,
    k: int = 5,
    split_file_path: str = "Raw Data/post_id_divisions.json",
) -> Dict[str, str]:
    """
    ...
    eraser_dir = os.path.join(
        output_dir, "eraser_ground_truth", safe_model_name, task_type
    )
    ...
    print(
        f"Converting {len(df)} entries to ERASER ground truth format (top-{k} approach)..."
    )

    for idx, row in df.iterrows():
        eraser_entry = convert_entry_to_eraser_ground_truth(
            row.to_dict(), tokenizer, k=k
        )
...
def create_eraser_docs_directory(df, output_dir: str, model_name: str, task_type: str):
    ...
    docs_dir = os.path.join(
        output_dir, "eraser_ground_truth", safe_model_name, task_type, "docs"
    )
    ...
    for idx, row in df.iterrows():
        ...
        tokens = input_ids[0].tolist()  # if it's a batch of one
        tokens = [str(token) for token in tokens if token != 0]
    ...
    print(
        f"Created {len([f for f in os.listdir(docs_dir) if os.path.isfile(os.path.join(docs_dir, f))])} doc files"
    )
    print(f"Docs directory ready at: {docs_dir}")

def save_eraser_ground_truth(df_3class, df_2class, model_name: str, save_dir: str = 'preprocessed_data', 
                           k: int = 5, split_file_path: str = 'Raw Data/post_id_divisions.json'):
    """
    Save ERASER ground truth format alongside regular preprocessed data
    
    Args:
        df_3class: 3-class dataframe
        df_2class: 2-class dataframe  
    ...
    print(f"Creating ERASER ground truth for {model_name} (top-{k} approach)...")
    
    # Load tokenizer
    tokenizer = AutoTokenizer.from_pretrained(model_name)
    
    # Create ERASER ground truth for 3-class
    print("Creating 3-class ERASER ground truth...")
    ground_truth_3class_files = create_eraser_ground_truth_from_df(
        df_3class, tokenizer, save_dir, model_name, "3class", k=k, split_file_path=split_file_path
    )
    
    # Create docs directory for 3-class
    create_eraser_docs_directory(df_3class, save_dir, model_name, "3class")
    
    # Create ERASER ground truth for 2-class
    print("\nCreating 2-class ERASER ground truth...")
    ground_truth_2class_files = create_eraser_ground_truth_from_df(
        df_2class, tokenizer, save_dir, model_name, "binary", k=k, split_file_path=split_file_path
    )
    
    # Create docs directory for 2-class
    create_eraser_docs_directory(df_2class, save_dir, model_name, "binary")
    
    print("\nERASER ground truth creation complete!")
    
    return {
        "3class": ground_truth_3class_files,
        "binary": ground_truth_2class_files
    }