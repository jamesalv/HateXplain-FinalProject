...
def process_raw_entries(data: Dict[str, Any], tokenizer: Any) -> Tuple[List[Dict[str, Any]], int]:
    """
    ...
    for key, value in tqdm(data.items(), desc="Processing entries", unit="entry"):
        ...
        # Process Rationales
        processed_entry['inputs'], processed_entry['rationales'] = align_rationales(value['post_tokens'], value['rationales'], tokenizer)
...
def save_preprocessed_data(df_3class, df_2class, model_name, save_dir='preprocessed_data'):
    """Save preprocessed data for later use"""
    # Create model-specific directory
    model_save_dir = os.path.join(save_dir, model_name.replace('/', '_'))  # Replace '/' to avoid path issues
...
def preprocess_datasets(data_path: str, model_name: str, save_dir='preprocessed_data') -> Tuple[pd.DataFrame, pd.DataFrame]:
    """
